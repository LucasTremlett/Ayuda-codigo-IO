...
### Tabla cruzada detallada (como tabla dinamica de excel). Básica.
...
example %>% group_by(OCC2010, SEX) %>% summarise(wage=mean(median_wage)) %>% ungroup() %>% 
  ...
  pivot_wider(names_from = c(SEX), values_from=wage) %>% gt(rowname_col = "OCC2010") %>%
  tab_options(container.height = px(500)) %>% fmt_currency(columns = c(2,3), decimals = 0, currency="euro", sep_mark=".") %>% 
